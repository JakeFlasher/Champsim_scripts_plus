...
def calculate_harmonic_mean(miss_rates: dict[str, list[float]]) -> dict[str, float]:
    """Calculate harmonic mean of miss rates for each algorithm."""
    harmonic_means = {}
    for algo, rates in miss_rates.items():
        if rates:
            harmonic_means[algo] = hmean(rates)
    return harmonic_means

def plot_means(means: dict[str, dict[str, float]], output_dir: str, mode: str) -> None:
    """Plot harmonic means of miss rates for each algorithm and application."""
    ...
    with plt.style.context(['science', 'ieee']):
        ...
        ax.set_ylabel("Harmonic Mean of Miss Ratio", fontsize=14)
        ...
        fig.savefig(os.path.join(output_dir, f"harmonic_mean_plot_{suffix}.pdf"), bbox_inches="tight")
        plt.close(fig)
        print(f"Plot is saved to {os.path.join(output_dir, f'harmonic_mean_plot_{suffix}.pdf')}")

def log_means(means: dict[str, dict[str, float]], log_file: str, mode: str) -> None:
    """Log harmonic means to a file."""
    with open(log_file, 'w') as f:
        for algo, app_means in means.items():
            for app, mean in app_means.items():
                f.write(f"{algo}, {app}, {mean:.4f}\n")
    print(f"Harmonic means logged to {log_file}")
...
def run(directory: str, mode: str):
    """Read all CSV files in the directory, calculate harmonic means, and plot."""
    ...
    for filepath in csv_files:
        ...
        app_means = calculate_harmonic_mean(miss_rates)
        
        for algo, mean in app_means.items():
            means[algo][app_name] = mean
    
    plot_means(means, directory, suffix)
    log_means(means, os.path.join(directory, f"harmonic_means_{suffix}.log"), suffix)
