...
TRACE_DIR="/root/google_traces/server_pre_trained_0.75"
...
SIMULATOR="/root/ChampSimFix/bin/champsim_bingo"
...
run_simulator() {
  ...
  local SIMULATION_INSTRUCTIONS_FOLDER=rowhammder_dev/pre_trained_bingo_0.75
  ...
  ${SIMULATOR} --warmup_instructions ${WARMUP_INSTRUCTIONS}   "${TRACE_FILE}" > "${RESULTS_DIR}/${TRACE_FILENAME}.out"
}
...
for TRACE_FILE in $(ls ${TRACE_DIR}/*.{gz,xz}); do
