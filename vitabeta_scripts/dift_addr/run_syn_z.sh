...
TRACE_DIR="/root/champsim_traces/gap"
...
TARGET_FOLDER=$2
MAX_JOBS=2  # Maximum number of parallel jobs
...
run_simulator() {
  ...
  local DAT_FOLDER=${TARGET_FOLDER}
    ...
    elif [[ "$TRACE_FILENAME" == *.xz ]]; then
        APP_NAME="${TRACE_FILENAME%.xz}"
    else
  ...
  ${SIMULATOR} "${TRACE_FILE}" ${WARMUP_INSTRUCTIONS}  ${SIMULATION_INSTRUCTIONS} 100000000 "${DAT_DIR}/${TRACE_FILENAME}.syn.xz" "/root/imputation/pypots/gap/wndw_ar/${APP_NAME}/change_points.log" 0.5 
...
for TRACE_FILE in $(ls ${TRACE_DIR}/*.gz); do
