...
champsim_trace_decoder::decode(const input_instr& input)
{
  ...
  if (input.is_branch) {
    // Modify opcode based on branch_taken
    if (input.branch_taken) {
      ins_.op = propagator::instr::opcode::OP_BRANCH_TAKEN;
    } else {
      ins_.op = propagator::instr::opcode::OP_BRANCH_NOT_TAKEN;
    }
    return ins_;
  }
