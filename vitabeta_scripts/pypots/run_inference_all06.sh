...
for TRACE_FILE in "$SOURCE_TRACE_PATH/"*.xz; do
    ...
    DATA_PATH="/root/developing/champsim_traces_dev/spec06/IPC_single_instr/500M/torch_data_v1/${APP_NAME}.trace.xz.csv"  # Replace with actual path
    ...
    # COMMAND="python load_pypots_chunks.py --data_path $DATA_PATH --model $MODEL_NAME --model_path $MODEL_PATH --chunk_size 100000000 --save_path $SAVE_PATH --device $DEVICE > $LOG_FILE 2>&1"
    ...
    echo "COMMAND="python load_pypots_chunks.py --data_path $DATA_PATH --model $MODEL_NAME --model_path $MODEL_PATH --save_path $SAVE_PATH --device $DEVICE > $LOG_FILE 2>&1""
