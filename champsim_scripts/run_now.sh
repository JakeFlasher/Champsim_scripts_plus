...
TRACE_DIR="/root/ChampSim/dpc3_traces"

# Simulator command and parameters
SIMULATOR="python decode_all_skip.py"
WARMUP_INSTRUCTIONS=30000000
SIMULATION_INSTRUCTIONS_LIST=(100000000)  # Add more if needed
MAX_JOBS=24  # Maximum number of parallel jobs
...
run_simulator() {
  ...
  local SIMULATION_INSTRUCTIONS_FOLDER=$((${SIMULATION_INSTRUCTIONS} / 1000000))M
  ...
  ${SIMULATOR} "${TRACE_FILE}" "${RESULTS_DIR}/${TRACE_FILENAME}.csv"  ${SIMULATION_INSTRUCTIONS} ${WARMUP_INSTRUCTIONS}
