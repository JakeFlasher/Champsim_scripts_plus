...
run_simulator_limit() {
  ...
  # Run the simulator command
   echo "${SIMULATOR} -w ${WARMUP_INSTRUCTIONS}  "${TRACE_FILE}" > "${OUTPUT_FILE}""
...
for TRACE_FILE in "${MODEL_TRACE_DIR}"/*.{xz,gz}; do
...
for TRACE_FILE in "${REFERENCE_TRACE_DIR}"/*.{xz,gz}; do
  # Get the base filename
  TRACE_FILENAME=$(basename "${TRACE_FILE}")
  # Remove the extension to get the application name
  APP_NAME="${TRACE_FILENAME%.*}"
