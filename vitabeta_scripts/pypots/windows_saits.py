...
def process_file(file_path, data_dir, output_dir, rel_path, plot_flag):
    ...
    # Read CSV file
    try:
        df = pd.read_csv(file_path,usecols=["ipc"], engine="pyarrow")
    ...
    # Change Point Detection using Window-based method
    try:
        model = 'ar'  # Using 'l2' cost function suitable for mean shifts
        # Estimate window size
        total_length = len(ipc_values)
        width = 32768  # Adjust based on your data
        if width > total_length // 2:
            width = total_length // 2 - 1

        # Initialize Window instance
        algo = rpt.Window(width=width, model=model, jump=50000).fit(ipc_values)


        # Determine penalty value
        ipc_diff_std = np.std(np.diff(ipc_values))
        penalty_value = np.log(total_length) * ipc_diff_std ** 2  # Adjust multiplier as needed
