...
# SIMULATOR="/root/GST_champsim/rowhammer_champ/bin/ramulator_syn_noprefetch"
# SIMULATOR="/root/developing/rowhammerRam_multicore/bin/champsim"
SIMULATOR="/root/developing/rowhammerRam/bin/champsim_1_interval"
...
run_simulator() {
  ...
  local SIMULATION_INSTRUCTIONS_FOLDER=IPC_single_instr/$((${SIMULATION_INSTRUCTIONS} / 1000000))M
  ...
  ${SIMULATOR} -w 0 -i ${SIMULATION_INSTRUCTIONS}   "${TRACE_FILE}" > "${RESULTS_DIR}/${TRACE_FILENAME}.out"
}
