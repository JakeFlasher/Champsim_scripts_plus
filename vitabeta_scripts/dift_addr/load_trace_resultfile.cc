...
void process_trace_file(const std::string &trace_file_path, const std::string &output_trace_file,
	...
	// set != or == to decide whether to keep those instr in the result log file or the reverse
...
std::unordered_set<size_t> load_stable_instructions(const std::string &stable_load_file) {
    ...
    if (!file.is_open()) {
        std::cerr << "Failed to open stable instructions file." << stable_load_file << std::endl;
    ...
    while (std::getline(file, line)) {
        if (std::regex_search(line, match, heartbeat_regex)){
            ...
            stable_instructions.insert(instruction_idx);;
        }
    }
