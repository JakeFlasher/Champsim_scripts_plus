...
def process_file(file_path, data_dir, output_dir, rel_path, plot_flag):
    ...
    try:
        ...
        algo = rpt.KernelCPD(kernel="linear",min_size=min_size, jump=jump).fit(ipc_values)

        # Determine penalty value
        ipc_var = np.var(ipc_values)
        penalty_value = 3 * np.log(len(ipc_values)) * ipc_var  # Adjust multiplier as needed
